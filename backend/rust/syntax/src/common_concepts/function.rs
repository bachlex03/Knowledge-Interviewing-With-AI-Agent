// Refs:
// https://doc.rust-lang.org/stable/book/ch03-03-how-functions-work.html?highlight=function#functions
...
fn main() {
    println!("=== Functions and Closures in Rust ===\n");

    // 1. Basic Functions
    demonstrate_basic_functions();

    // 2. Methods and Associated Functions (impl Keyword)
    ...
    // 3. Closures: Basics and Syntax
    demonstrate_closure_basics();

    // 4. Closures: Capturing
    demonstrate_closure_capturing();

    // 5. Closures: As Input Parameters
    demonstrate_closure_as_input();

    // 6. Closures: Input Functions
    demonstrate_input_functions();

    // 7. Closures: As Output Parameters
    demonstrate_closure_as_output();

    // 8. Closures: Std Library Examples (any, find)
    demonstrate_std_closure_examples();
}

// 1. Basic Functions
fn demonstrate_basic_functions() {
    println!("1. Basic Functions");
    println!("   Functions use 'fn' keyword, type-annotated parameters, and return types\n");

    // Statements vs Expressions
    // Statements: Perform actions, do not return a value (end with ;)
    // Expressions: Evaluate to a resulting value (no semicolon)
...
// 2. Basic methods with &self
fn demonstrate_basic_methods() {
    println!("2. Basic Methods with &self");
...
// 2.1 Mutable methods with &mut self
fn demonstrate_mutable_methods() {
    println!("2.1 Mutable Methods with &mut self");
...
// 2.2 Methods with multiple parameters
fn demonstrate_methods_with_parameters() {
    println!("2.2 Methods with Multiple Parameters");
...
// 2.3 Associated functions (no self)
fn demonstrate_associated_functions() {
    println!("2.3 Associated Functions");
...
// 2.4 Method calls as syntactic sugar
fn demonstrate_method_syntax_sugar() {
    println!("2.4 Method Calls as Syntactic Sugar");
...
// 2.5 Multiple impl blocks
fn demonstrate_multiple_impl_blocks() {
    println!("2.5 Multiple impl Blocks");
...
// 2.6 Getters and method naming
fn demonstrate_getters() {
    println!("2.6 Getters and Method Naming");
...
// 2.7 Methods that consume self
fn demonstrate_consuming_methods() {
    println!("2.7 Methods that Consume self");
...
// 3. Closures: Basics and Syntax
fn demonstrate_closure_basics() {
    println!("3. Closure Basics");
    println!("   Anonymous functions that can capture their environment\n");

    let x = 1;

    // Full syntax
    let plus_one = |i: i32| -> i32 { i + x };

    // Type inference (optional brackets for single line)
    let plus_two = |i| i + x + 1;
...
// 4. Closures: Capturing
fn demonstrate_closure_capturing() {
    println!("4. Closure Capturing");
    println!("   Closures can capture by reference (&T), mut reference (&mut T), or value (T)\n");
    ...
    // Requires 'mut' on closure variable
    let mut inc = || {
        count += 1;
    ...
    // 'move' keyword forces capturing by value
    let haystack = vec![1, 2, 3];
    let contains = move |needle| haystack.contains(needle);

    println!("   Contains 2? {}", contains(&2));
    // println!("{:?}", haystack); // Error! `haystack` was moved into the closure
    println!();
}

// 5. Closures: As Input Parameters
fn demonstrate_closure_as_input() {
    println!("5. Closures as Input Parameters");
    println!("   Traits: Fn (immutable), FnMut (mutable), FnOnce (consumes)\n");

    // Takes a closure that implements Fn (can be called multiple times)
    fn apply<F>(f: F)
    where
        F: Fn(),
    {
    ...
    let greeting = "Hello";
    let diary = || println!("   Diary entry: {}", greeting);
...
// 6. Closures: Input Functions
fn demonstrate_input_functions() {
    println!("6. Input Functions");
    println!("   Regular functions can be passed where a closure is expected\n");
    ...
    call_me(closure);
    call_me(function); // Passing function as parameter
    println!();
}

// 7. Closures: As Output Parameters
fn demonstrate_closure_as_output() {
    println!("7. Closures as Output Parameters");
    println!("   Use 'impl Fn' and 'move' to return a closure\n");
    ...
    let double = create_fn();
    println!("   Result of returned closure: {}", double(5));
    println!();
}

// 8. Closures: Std Library Examples
fn demonstrate_std_closure_examples() {
    println!("8. Standard Library Examples");
    println!("   Using closures with iterators (any, find)\n");

    let vec = vec![1, 2, 3];

    // Iterator::any takes a closure that returns bool
    // It takes a reference to each element
    let has_two = vec.iter().any(|&x| x == 2);
    println!("   Vec has 2? {}", has_two);

    // Iterator::find returns an Option
    let found = vec.iter().find(|&&x| x > 2);
    println!("   Find x > 2: {:?}", found);
    println!();
}
